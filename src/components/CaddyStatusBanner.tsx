...
import { useService, useInstallService, useStartService } from '@/api/services/services-queries';
...
export default function CaddyStatusBanner() {
  ...
  if (!isDockerRunning) {
    return (
      <div className="sticky top-0 z-50 flex items-center justify-between gap-3 bg-amber-500/10 backdrop-blur-sm border-b border-amber-500/20 px-4 py-1.5">
    ...
    <div className="sticky top-0 z-50 flex items-center justify-between gap-3 bg-amber-500/10 backdrop-blur-sm border-b border-amber-500/20 px-4 py-1.5">
      ...
      {isCaddyInstalled ? (
        <Button
          size="sm"
          variant="ghost"
          className="h-5 px-2 text-[10px] font-medium text-amber-700 dark:text-amber-300 hover:bg-amber-500/20"
          onClick={handleStart}
