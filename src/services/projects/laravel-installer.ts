...
import type { LaravelInstallerOptions, VolumeCopyProgress } from '../../types/project';
...
async function flattenVolumeStructure(
  ...
  try {
    console.log(`Flattening volume structure for ${projectName}...`);
    if (onProgress) {
      onProgress({
    ...
    try {
      ...
      console.log('Successfully moved Laravel files to volume root');
    } finally {
      await container.remove();
    }
  } catch (error) {
    console.error('Failed to flatten volume structure:', error);
...
export async function installLaravelToVolume(
  ...
  try {
    ...
    console.log('Pulling Laravel installer image...');
    if (onProgress) {
      onProgress({
    ...
    console.log(`Installing Laravel project: ${projectName}`);
    if (onProgress) {
      onProgress({
    ...
    try {
      ...
      console.log('Laravel installation completed');
    } finally {
      await container.remove();
    }
    ...
    console.log(`Successfully installed Laravel to volume ${volumeName} (flat structure)`);
  } catch (error) {
    console.error('Failed to install Laravel:', error);
